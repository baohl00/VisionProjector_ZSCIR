...
class original_gMLP(nn.Module):
    ...
    def __call__(self, x) -> jnp.ndarray:
        shortcut = x
        #new_features = self.features * self.expansion_rate
        #x = nn.Dense(self.features)(x)
        x = nn.LayerNorm()(x)
        attn_out = None
...
class gMLP(nn.Module):
    ...
    def __call__(self, x) -> jnp.ndarray:
        shortcut = x
        x_gmlp = original_gMLP(
                features = self.features
                )(x)
        new_features = self.features * self.expansion_rate
        x = nn.Dense(new_features)(x)
        x = nn.LayerNorm()(x)
        attn_out = None
        if self.attn_features:
            attn_out = TinyAttn(
                    features=new_features // 2, attn_features=self.attn_features
                    )(x)
        x = nn.Dense(new_features)(x)
        x = nn.sigmoid(x)
        x = SGU()(x, attn_out)
        ...
        output = shortcut + x 
        if self.combine:
            output = (output + x_gmlp)/2
        return output #(shortcut + x) if  #+ x_gmlp)/2
...
class TinyAttn(nn.Module):
    """
    Tiny single headed attention for content aware (as opposed to purely spatial) gating.
    """
    features: int
    attn_features: int
    ...
    def __call__(self, x) -> jnp.ndarray:
        ...
        #print(q.shape, k.shape, v.shape, x.shape)
