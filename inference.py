...
from data_utils import (
        build_circo_dataset, build_fiq_dataset, build_cirr_dataset
        )
...
if __name__ == "__main__":
  ...
  parser.add_argument(
      ...
      choices=["fiq-dress", "fiq-shirt", "fiq-toptee", "circo", "dtin", "cirr"],
  )
  parser.add_argument(
      "--output",
      type=str,
  ...
  parser.add_argument(
      "--top_k", type=int, default=50, help="Top K."
  )
  parser.add_argument(
      "--note", type=str, default=None, help="Note for output."
  )
  ...
  if args.dataset.startswith("fiq"):
    ...
    eval_dataset = build_cirr_dataset(
        dataset_name = args.dataset, tokenizer = tokenizer
        )
  else:
  ...
  for i in tqdm(range(0, len(eval_dataset.query_examples), args.batch_size)):
    ...
    top_k_indices = jnp.argsort(similarity_scores, axis=1)[:, -args.top_k:][:, ::-1]
    top_k_iids = [
  ...
  elif args.dataset in ["circo", "cirr"]:
    eval_dataset.write_to_file(
        os.path.join(args.output, f"{args.dataset}_{args.model_size}_{args.top_k}{args.note}")
    )
  else:
    raise NotImplementedError
