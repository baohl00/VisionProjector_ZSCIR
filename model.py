...
from layers import AttenTokenPoolingLayer, StackedTransformer, gMLP, ResidualMLP, MLP, Phi, TextProj, original_gMLP
...
class MagicLens(nn.Module):
  ...
  def clip_encode(self, input_batch: Dict) -> Tuple[jax.Array, jax.Array]:
    ...
    # Applying Projector for image
    og_gmlp = original_gMLP(
            features = MagicLensConfig[self.model_size]['embed_dim'],
            attn_features = None
            )
    gmlp = gMLP( 
            features = MagicLensConfig[self.model_size]['embed_dim'], 
            attn_features = None, #MagicLensConfig[self.model_size]['embed_dim']//8
            expansion_rate = 8,
            combine = True 
            )
    ...
    resmlp = ResidualMLP(
            ...
            rate = 0.5, 
            n=4)
    mlp = MLP(expansion_rate = 12) 
    phi = Phi(
            input_dim = MagicLensConfig[self.model_size]['embed_dim'], 
            hidden_dim = MagicLensConfig[self.model_size]['embed_dim'] * 1, 
            output_dim = MagicLensConfig[self.model_size]['embed_dim'], 
            dropout_rate = 0.5)
    ...
    img_var = proj_img.init(rng, image_embs) 
    image_embs = proj_img.apply(img_var, image_embs)
    #img_var = proj_txt.init(rng1, image_embs) 
    #image_embs = proj_txt.apply(img_var, image_embs)
    txt_var = proj_txt.init(rng1, text_embs) 
    #text_embs = proj_txt.apply(txt_var, text_embs)
